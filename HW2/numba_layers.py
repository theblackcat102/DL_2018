...
class Conv(Layers):
    ...
    def forward(self, x):
        ...
        def multiply(X_col, W_col, bias, filters, H_out, W_out, N):
            X_col_output = W_col.dot(X_col) + bias
            output = X_col_output.reshape(filters, H_out, W_out, N)
        ...
        # print(output.shape)
        return multiply(X_col, W_col, self.bias, self.filters, H_out, W_out, N)
...
class Sigmoid(Layers):
    ...
    def backward(self, grad):
        def sigmoid_back(output, grad):
            return grad * (output - output**2)
        output = sigmoid_back(self.output, grad)
        return output
